        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-[10px] tracking-[0.18em] text-label uppercase">
              Finances &amp; facturation · Suivi du revenu
            </span>
          </div>
          <h1 className="text-[28px] font-bold leading-tight tracking-[-0.02em] text-ink">Chiffre d&apos;affaires</h1>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={load}
            ...
            <tbody>
              {billables.slice(0, 10).map((b) => (
                <tr key={b.key} className="border-b border-line last:border-0">
                  <td className="p-4">
                    <div className="flex items-center gap-2">
                      <span className={`rounded-full px-2 py-0.5 text-[9px] font-bold uppercase ${b.type === "navette" ? "bg-blue-50 text-blue-700" : "bg-slate-100 text-slate-700"}`}>
                        {b.type === "navette" ? "Navette" : "Course"}
                      </span>
                  ...
                  <td className="p-4">
                    <span className="rounded-full bg-paper px-2 py-0.5 text-[10px] font-bold text-muted">{b.status}</span>
                  </td>
...
function StatCard({ label, value, sub, accent = false }: { label: string; value: string; sub?: string; accent?: boolean }) {
  return (
    <div className={`rounded-2xl border p-5 ${accent ? "border-accent/30 bg-accent/[0.04]" : "border-line bg-white"}`}>
      ...
      <div className={`mt-2 font-mono text-[28px] font-bold leading-none tracking-[-0.02em] tabular-nums ${accent ? "text-accent" : "text-ink"}`}>{value}</div>
      {sub && (

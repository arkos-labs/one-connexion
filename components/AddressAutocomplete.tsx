...
export function AddressAutocomplete({
  ...
  useEffect(() => {
    const fetchAddresses = async () => {
      ...
      setIsLoading(true);
      try {
        // On limite la recherche à 15 pour être sûr d'avoir des résultats après le filtrage
        const res = await fetch(`https://api-adresse.data.gouv.fr/search/?q=${encodeURIComponent(query)}&lat=48.8566&lon=2.3522&limit=15`);
  ...
  return (
    <div ref={wrapperRef} className="relative w-full">
      ...
      {isOpen && suggestions.length > 0 && (
        <ul className="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-xl border border-line bg-white py-1 shadow-lg">
